...
[To be filled from experiments. Claims, scoped strictly to feasibility:
- **Discrimination (E3):** simulated learners of graded competence (expert→careless) receive grades
  monotone in competence on each skill — the instrument has signal.
- **Dissociation (E1):** across the generated population, inter-skill grade correlations are modest
  and the grade matrix does not collapse to a single factor — the three constructs are separable
  (supports P3).
- **Reliability (E2):** grader test-retest at production temperatures; grade-flip rate and
  per-criterion variance, addressing LLM-judge self-inconsistency.
NO learning-outcome claims. Simulated learners are labeled as apparatus; the human-rater validation
study (Section 10) is presented as prepared, not completed.]
