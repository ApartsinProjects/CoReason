...
learners. At seven grader calls each (three skill evaluators, a grading pass per skill, and one
steering-update call) the nominal total is 280; 255 calls were actually issued because grading inputs
that are identical across profiles are cached and reused. Framing and Steering responses are generated
...
near-zero cross-effects, so a simulated learner's three model-assigned grades move independently. Each
...
The own-skill effects are directionally consistent across subjects: broken down by the five subject
areas (eight learners each), Framing and Steering each show a positive own-competence effect in four of
five subjects, with the two exceptions (Framing in statistics, Steering in microeconomics) reflecting
