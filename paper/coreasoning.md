...
*discriminates* competence, that the three skills *dissociate*, and that it is *reliable* enough to
report. We make no learning-outcome claims. The "learners" are simulated personas of controlled
competence (expert, proficient, novice, careless), generated by a separate model instructed to write
exactly as a student of that level would; they are experimental apparatus, not human participants. The
validation against human expert graders is prepared but not yet run (Section 10).

**Discrimination.** Grading a held-out challenge with an expert and a careless persona yields the
expected separation on every skill: the expert receives Framing B, Judging A, Steering B, while the
careless persona receives C on all three. The judging signal is mechanistically transparent: the
expert flags all four seeded issues with no false alarms (perfect recall and precision), and the
careless persona flags none of the four while raising two false issues, which the grader maps to A and
C respectively. [Full four-level results across multiple challenges, showing grades monotone in
competence per skill, are reported in Table X; data and code accompany the paper.]

**Dissociation.** [Across the generated population we report inter-skill grade correlations and a
factor analysis; the central claim, supporting Proposition P3, is that the three grades do not
collapse to a single factor, so a persona can score differently across skills. Numbers forthcoming
from the scaled run.]

**Reliability.** [We re-grade fixed transcripts repeatedly at the production temperatures and report
the grade-flip rate and per-criterion variance per skill, addressing the documented self-inconsistency
of LLM judges. Numbers forthcoming.]

These results establish feasibility only: the constructs are separable and can be scored
automatically. Whether the automated grades agree with expert human judgment is the subject of the
validation study in Section 10.
