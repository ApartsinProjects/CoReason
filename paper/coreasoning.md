...
entirely separate mechanisms. The separation holds under a second grader backend, and the grader is
internally consistent on repeat, which is a precision check rather than accuracy against human experts.
We make no learning-outcome claims: because the learners are simulated and the grades are not yet
validated against human raters, this establishes that the constructs are *separable and automatically
scoreable under controlled conditions*, not that the scores match expert judgment or that students
learn. The instrument, data, and a prepared human-rater validation protocol are publicly available.
...
subject. These per-subject splits are illustrative rather than powered comparisons, given only eight
learners per subject; the load-bearing estimates are the pooled own-effects and their confidence
intervals above. The designed-contrast personas make the separation concrete (Figure 4, which plots five
...
**Grader-backend robustness (within family).** All grades above come from a single grader model
(gpt-4o), while the prototype's engine was a different model (llama-3.3-70b). To probe whether the
dissociation is an artifact of one grader, we re-graded the identical 40 transcripts (challenges and
learner responses held fixed) with a different model, gpt-4o-mini. The dissociation replicates:
own-skill effects again dominate cross-skill effects (diagonal +0.47 versus off-diagonal +0.07). Notably, under this weaker
...
the Proposition-P2 ceiling relation discussed below rather than absence of the construct.) Both graders
are from a single provider family, so this establishes within-family robustness only; replication across
an independent vendor is a deliberate open item in the validation program (Section 10).
