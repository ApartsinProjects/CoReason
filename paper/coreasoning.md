...
supplied to the judging evaluator) versus the model's own judgment, we re-graded the original
40-learner subset with that scaffolding removed (comparing against that subset's own baseline, where
the effects are Framing +0.60, Judging +2.00, Steering +0.50). The effect is sharply skill-specific. Framing discrimination is essentially
